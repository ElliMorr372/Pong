...
namespace Pong
{
    public partial class Form1 : Form
    {
        ...
        private void gameTimer_Tick(object sender, EventArgs e)
        {
            ...
            //check if the ball hit top or bottom wall and change direction if it does
            if (ball.Y < 0 || ball.Y > this.Height - ball.Height)
            {
                ballYSpeed *= -1; // or: ballYSpeed = -ballYSpeed;
            }

            //check if ball hits either play. IF it does change the direction
            ...
            //check if a player missed the ball if true add 1 to scor of other player
            if (ball.X < 0)
            {
                player2Score++;
